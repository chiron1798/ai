...
## 知识库同步（重要：每次任务开始前执行）
本智能体的所有业务规则、技能定义、产品数据均不在本地固定文件中，而是维护在以下GitHub仓库（纯文本，可直接WebFetch读取，无需JS渲染）：

- 技能索引：`https://raw.githubusercontent.com/<你的用户名>/<仓库名>/main/skills-index.csv`
- 各技能正文：`https://raw.githubusercontent.com/<你的用户名>/<仓库名>/main/skills/<文件名>.md`（具体路径见技能索引中的"文件路径"列）
- 产品线数据：`https://raw.githubusercontent.com/<你的用户名>/<仓库名>/main/product-lines/product-lines.csv`
- 版本记录：`https://raw.githubusercontent.com/<你的用户名>/<仓库名>/main/version-log.csv`
